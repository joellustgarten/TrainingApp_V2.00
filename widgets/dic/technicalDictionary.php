...
require_once('../../src/content/config.php'); // Include the database configuration file
...
try {
	...
	if (isset($_SESSION['selected_meaning'])) {
		...
		// Query database with the submitted term
		$searchTerm = "SELECT * FROM technical_dictionary 
                       WHERE PORTUGUESE LIKE :term 
                          OR INGLES LIKE :term 
                          OR ALEMAN LIKE :term 
	...
	// Fetch and build table rows
	if ($stmt && $number > 0) {
		$result = $stmt->fetchAll(PDO::FETCH_ASSOC);
		foreach ($result as $data) {
			$rows .= "<tr>
                        <td>" . htmlspecialchars($data['PORTUGUESE']) . "</td>
                        <td>" . htmlspecialchars($data['INGLES']) . "</td>
                        <td>" . htmlspecialchars($data['ALEMAN']) . "</td>
                        <td>" . htmlspecialchars($data['ESPANOL']) . "</td>
                      </tr>";
		}
	} elseif (isset($stmt) && $number === 0) {
		// Show "No records found" if no results found
...
<head>
	...
	<title>TrainingApp | Technical Dictionary</title>
	...
	<header class="o-header">
		...
		<div class="e-container">
			<div class="o-header__meta">
				...
				<span class="o-header__subbrand" data-i18n="dict_title"></span>
			</div>
		</div>
		...
		<div class="main_container">
			<div id="index_container" class="i_container">
				...
				<div class="table_container">
					<table class="m-table">
						<caption data-i18n="dict_caption"></caption>
						<thead>
							<tr>
								<th>Portuguese</th>
								<th>English</th>
