...
if (isset($_POST['submit'])) {
    ...
    if (!empty($term)) {
        ...
        try {
            // Assuming $pdo is your PDO connection from config.php
            $stmt = $pdo->prepare("SELECT abbreviation, meaning FROM abreviations WHERE abbreviation LIKE :term");
            $stmt->execute(['term' => '%' . $term . '%']); // Use prepared statement
            $result = $stmt->fetchAll(PDO::FETCH_ASSOC);
            $number = $stmt->rowCount(); // Get the number of rows
            if ($result) {
                foreach ($result as $data) {
                    $rows .= "<tr>
                        <td>" . htmlspecialchars($data['abbreviation']) . "</td>
                        <td>" . htmlspecialchars($data['meaning']) . "</td>
                        <td><form action='' method='POST' style='display:inline;'>
                                <input type='hidden' name='selected_meaning' value='" . htmlspecialchars($data['meaning']) . "'>
                                <button type='submit' name='set_translation' style='all:unset; box-sizing:border-box; display: inline-block; cursor:pointer;'>Translation &gt;</button>
                            </form>
                            </td>
                    </tr>";
                }
            ...
            $rows = '<tr><td colspan="3">Error fetching data. Please try again later.</td></tr>'; // Generic error message
        }
    } else {
        $rows = '<tr><td colspan="3">Please provide the search term.</td></tr>';
    }
} else {
    $rows = '<tr><td colspan="3">Please provide the search term.</td></tr>';
}
                ...
                <div class="o-header__top">
                    ...
                    <div class="o-header__search">
                        ...
                        <button
                            ...
                            <span class="a-button__label" data-i18n="search"></span>
                        </button>
                    </div>
                    <button
                        type="button"
        ...
        <div class="main_container">
            <div id="index_container" class="i_container">
                <div class="number" style="text-align:center;">
                    <h4><span data-i18n="hits"></span><?php echo $number; ?></h4>
                </div>
                <div class="table_container">
                    <table class="m-table">
                        <caption data-i18n="acron_caption"></caption>
                        <thead>
                            <tr>
                                <th data-i18n="acron_title"></th>
                                <th data-i18n="acron_meaning"></th>
                                <th data-i18n="acron_trans"></th>
                            </tr>
                        </thead>
                        <tbody>
                            <?php
